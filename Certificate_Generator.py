...
# Resizing logo image
logo = Image.open('logo_color.png')
basewidth = 200
...
program_font = ImageFont.truetype('Fonts\\Montserrat-Bold.otf', size=36)
date_font = ImageFont.truetype('Fonts\\Montserrat-Regular.otf', size=32)
...
for name in participants:
    Blank_sheet = Image.open('Temp.png')
    draw = ImageDraw.Draw(Blank_sheet)
    draw.text((290, 275), program, (37,54,78), font=program_font)
    draw.text((330,320), date, (37,54,78), font=date_font)
    draw.text((200,175), name, (37,54,78), font=program_font)
    Blank_sheet.save(f"{name}.png")
=======
# -*- coding: utf-8 -*-
"""
@author: Ishan Rawat
...
# Creating Template
Blank_sheet = Image.open('background.png')  # Your background file

# Resizing logo image
logo = Image.open('logo_color.png') # Your Logo
basewidth = 200
wpercent = (basewidth/float(logo.width))
relative_height = int((float(logo.height)*float(wpercent)))
logo = logo.resize((basewidth,relative_height) , Image.ANTIALIAS)
...
>>>>>>> 9dd948bf12049dd2b2afbeaf6c9cf67487a92d9b
